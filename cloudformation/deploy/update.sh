aws cloudformation update-stack --stack-name "AsteroidsBackendNetworkStack" --template-body file://../asteroids-backend-network.yaml
aws cloudformation update-stack --stack-name "AsteroidsBackendAuthenticationStack" --template-body file://../asteroids-backend-authentication.yaml --parameters ParameterKey=CognitoDomain,ParameterValue=chuckbenger
...
aws cloudformation update-stack --stack-name "AsteroidsBackendApplicationStack" --template-body file://../output/asteroids-backend-application.yaml --capabilities CAPABILITY_AUTO_EXPAND CAPABILITY_IAM --parameters ParameterKey=AuthenticationStackName,ParameterValue=AsteroidsBackendAuthenticationStack